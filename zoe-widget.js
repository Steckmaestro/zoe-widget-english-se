// CREDITS TO MOUNTBATT@GITHUB - This is just a modified version to fit Swedish language&codes
//https://gist.github.com/mountbatt/772e4512089802a2aa2622058dd1ded7?fbclid=IwAR2Uj6vgVXKEM6XQiMr0v9noQrXxFaAHSXc_J4yEN4ue_T89inQnA4Q01OY
...
async function createWidget(items) {
		...
		if(typeof(data.locationStatus) != 'undefined'){
			...
			const LocationVal = LocationStack.addText("➤ Open maps")
		...
		//if(typeof(data.hvacStatus) != 'undefined'){ // we have to uncomment this later!
			...
			const AcLabel = AcStack.addText("Pre. Climate")
			...
			if(args.queryParameters.action == 'start_ac'){
					AcVal = AcStack.addText("➤ Stop climate")
					ac_url = `scriptable:///run?scriptName=${scriptName}&action=stop_ac`; 
			} else {
				  AcVal = AcStack.addText("➤ Start climate")
